...
REPLY_MAP = dict_merge(
    string_keys_to_dict(
        'AUTH BGREWRITEAOF BGSAVE DEL EXISTS '
        'EXPIRE HDEL HEXISTS '
        'HMSET MOVE PERSIST RENAMENX SISMEMBER SMOVE '
        'SETEX SAVE SETNX MSET', reply_to_bool),
    string_keys_to_dict(
        'BITCOUNT DECRBY GETBIT HLEN INCRBY LINSERT '
        'LPUSHX RPUSHX SADD SCARD SDIFFSTORE SETBIT SETRANGE '
        'SINTERSTORE STRLEN SUNIONSTORE SETRANGE', reply_int),
    string_keys_to_dict(
        'FLUSHALL FLUSHDB SELECT SET SETEX '
        'SHUTDOWN RENAME RENAMENX WATCH UNWATCH', make_reply_assert_msg('OK')),
    ...
    string_keys_to_dict(
        ...
        'PSUBSCRIBE UNSUBSCRIBE', reply_pubsub_message),
...
class Client(object):
    ...
    def format_reply(self, cmd_line, data):
        if cmd_line.cmd not in REPLY_MAP:
            return data
        try:
            res = REPLY_MAP[cmd_line.cmd](data, *cmd_line.args,
                                          **cmd_line.kwargs)
    ...
    def execute_command(self, cmd, *args, **kwargs):
        ...
        while n_tries > 0:
            ...
            if not self.subscribed and cmd not in ('AUTH', 'SELECT'):
                if self.password and self.connection.info.get(
                        'pass', None) != self.password:
                    yield gen.Task(self.auth, self.password)
                if self.selected_db and self.connection.info.get(
                        'db', 0) != self.selected_db:
            ...
            listening = ((cmd in PUB_SUB_COMMANDS)
                         or (self.subscribed and cmd == 'PUBLISH'))
            if listening:
                result = True
    ...
    def zremrangebyrank(self, key, start, end, callback=None):
        self.execute_command('ZREMRANGEBYRANK',
                             key,
                             start,
                             end,
                             callback=callback)
    ...
    def zremrangebyscore(self, key, start, end, callback=None):
        self.execute_command('ZREMRANGEBYSCORE',
                             key,
                             start,
                             end,
                             callback=callback)
    ...
    def hincrbyfloat(self, key, field, amount=1.0, callback=None):
        self.execute_command('HINCRBYFLOAT',
                             key,
                             field,
                             amount,
                             callback=callback)
    ...
    def _subscribe(self, cmd, channels, callback=None):
        if isinstance(channels, str) or (not PY3
                                         and isinstance(channels, unicode)):
            channels = [channels]
...
class Pipeline(Client):
    ...
    def execute_command(self, cmd, *args, **kwargs):
        if self.executing and cmd in ('AUTH', 'SELECT'):
            ...
            raise RequestError('Client is not supposed to issue '
    ...
    def format_pipeline_request(self, command_stack):
        return ''.join(
            self.format_command(c.cmd, *c.args, **c.kwargs)
    ...
    def execute(self, callback=None):
        ...
        try:
            if self.transactional:
                command_stack = ([CmdLine('MULTI')] + command_stack +
                                 [CmdLine('EXEC')])
            ...
            password_should_be_sent = (
                self.password
                and self.connection.info.get('pass', None) != self.password)
            if password_should_be_sent:
                yield gen.Task(self.auth, self.password)
            db_should_be_selected = (
                self.selected_db
                and self.connection.info.get('db', None) != self.selected_db)
...
class Lock(object):
    ...
    def acquire(self, blocking=True, callback=None):
        ...
        while 1:
            ...
            result = yield gen.Task(self.redis_client.setnx, self.lock_name,
                                    timeout_at)
            ...
            yield gen.Task(
                self.redis_client._io_loop.add_timeout,
